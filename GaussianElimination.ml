...
struct 
  let pi = atan(1.) *. 4.
  let cos (n: float) = cos n 
  let sin (n: float) = sin n 
  let sum (n1: float) (n2: float) = n1 +. n2
  let max (lst: float list) = 
    match List.rev (List.sort ~cmp:compare lst) with
    | [] -> None
    | hd :: _ -> Some hd		     
end;;

assert ((Math.max [3.; 100.; 4.; 6.; (-8.); (-10.); 1337.; 1337.1]) = 
	  Some 1337.1);;
assert ((Math.max [(-3.); (-100.); (-4.); (-6.); (-8.); (-10.); (-1337.); 
		   (-1337.1); (-1.1)]) = Some (-1.1));;
assert ((Math.sum 5. (-4.)) = 1.);;
assert ((Math.cos (Math.pi)) = -1.);;
assert (Float.abs (0. -. (Math.sin (Math.pi))) < 0.000001) ;;
...
struct
    ...
    and hometown = "Long Island, Manila"
    and house = "Lionel Court"
...
module StudentJayShar = (Jayshar : TF);;
